...
open class AddExportMainManifestTask : DefaultTask() {
    ...
    private fun exportedManifest(file: File, outBuilder: StringBuilder, isMain: Boolean) {
        ...
        val replaceAttr = applicationNode.attributes()["android:extractNativeLibs"]
        if (replaceAttr == null || replaceAttr == "false") {
            applicationNode.attributes()["android:extractNativeLibs"] = "true"
        }
